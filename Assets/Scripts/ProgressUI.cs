...
public class ProgressUI : MonoBehaviour
{
    ...
    private void RefreshScorebarUI()
    {
        ...
        {
            _sliderTextUI.text = "Рост в секунду " + (_clickerScore.experienceMultiplayer + YandexGame.savesData.roomLevel + 1).ToString();
        }else if (YandexGame.savesData.language.Equals("en"))
        {
            _sliderTextUI.text = "Growth per second " + (_clickerScore.experienceMultiplayer + YandexGame.savesData.roomLevel + 1).ToString();
        }
